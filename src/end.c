...
STATIC_DCL char *FDECL(dump_format_str,(char *));
...
register struct monst *mtmp;
{
	...
	if(mtmp->data == &mons[PM_GHOST]) {
		...
		Strcat(buf, m_monnam(mtmp));
	        ...
	        if ((is_racial(mtmp->data) || is_were(mtmp->data)) &&
		    strcmp(racial_prefix(mtmp), ""))
...
register struct attack *mattk;
{
	...
	if (mattk->aatyp == AT_ENGL && mattk->adtyp == AD_DGST) {
	    killer_format = NO_KILLER_PREFIX;
	    Strcpy(buf, "eaten by ");
	} else if (mattk->adtyp == AD_COLD) {
	    killer_format = NO_KILLER_PREFIX;
	    Strcpy(buf, "frozen by ");
	} else if (mattk->adtyp == AD_DISN) {
	    killer_format = NO_KILLER_PREFIX;
	    Strcpy(buf, "disintegrated by ");
	} else if (mattk->adtyp == AD_ELEC) {
	    killer_format = NO_KILLER_PREFIX;
	    Strcpy(buf, "shocked by ");
	} else if (mattk->adtyp == AD_ACID) {
	    killer_format = NO_KILLER_PREFIX;
	    Strcpy(buf, "dissolved by ");
	} else if (mattk->adtyp == AD_BHED) {
	    killer_format = NO_KILLER_PREFIX;
	    Strcpy(buf, "beheaded by ");
	} else {
	    killer_format = KILLED_BY;
	    Strcpy(buf, "");
	}
	Strcat(buf, done_in_name(mtmp));

	if (multi)
	{
	    Strcat(buf, ", while ");
	    if (multi_reason) Strcat(buf, multi_reason);
	    else Strcat(buf, "helpless");
	}

	killer = buf;
	if (mtmp->data->mlet == S_WRAITH)
		u.ugrave_arise = PM_WRAITH;
	else if (mtmp->data->mlet == S_MUMMY && !Upolyd)
		u.ugrave_arise = PM_MUMMY;
	else if (mtmp->data->mlet == S_VAMPIRE && !Upolyd)
...
boolean taken;
{
	...
	if (invent) {
		...
		{
#ifdef DUMP_LOG
			boolean want_disp = (ask2) ? TRUE: FALSE;
#endif
			struct obj *obj;

			int tmp = HHallucination;
			...
			HHallucination = 0;
	    		...
	    		if(taken)
				Sprintf(qbuf,
				"Do you want to see the identification state of what you had when you %s?",
					(how == QUIT) ? "quit" : "died");
			else
				Strcpy(qbuf,"Do you want your possessions' identification state identified?");
			if (ask2)
			{
			...
			container_contents(invent, TRUE, TRUE, c == 'y');
#ifdef DYWYPISI
			if (c == 'y') dump_ID_off();
#endif
...
struct obj *list;	/* inventory or container contents */
{
	...
	if (Has_contents(obj)) {
	    ...
	    if (obj->otyp == AMULET_OF_YENDOR) continue; /*counted elsewhere*/
...
# endif
	...
	if (/*bones_ok && */u.ugrave_arise < LOW_PM) {
		...
		} else if (is_stone(youmonst.data) &&
		           !strncmpi(killer, "crumbled,", 9)) {
			corpse = mksobj_at(ROCK, u.ux, u.uy, FALSE, FALSE);
			if (corpse) {
				int tmp = youmonst.data->cwt /
					objects[ROCK].oc_weight;
				if (tmp < 5) corpse->quan = rn2(tmp) + 1;
				else corpse->quan = rn1(10, tmp-5);
#ifdef INVISIBLE_OBJECTS
		...
		if (levl[u.ux][u.uy].typ == GRAVE) {
		    levl[u.ux][u.uy].disturbed = 1; /* bones grave */
		}
	    }
	}
...
# else
	if (wizard && bones_ok && (yn("Save bones?") != 'y'))
	    bones_ok = FALSE;
# endif /* PARANOID */
#endif /* WIZARD */
	if (how < GENOCIDED)
	    savebones(corpse, bones_ok);
...
void
container_contents(list, identified, all_containers, dywypisi)
struct obj *list;
boolean identified, all_containers, dywypisi;
#ifdef DUMP_LOG
{
...
void do_containerconts(list, identified, all_containers, want_dump, want_disp, dywypisi)
struct obj *list;
boolean identified, all_containers, want_dump, want_disp, dywypisi;
#endif
