...
register const char *let,*word;
{
	...
	if(*let == WEAPON_CLASS &&
	   !strcmp(word, "throw") &&
	    maybe_polyd(throws_rocks(youmonst.data), Race_if(PM_GIANT)))
	    useboulder = TRUE;
...
nextclass:
	...
	for(i = 0; i < n; i++) {
	  otmp = oarray[i];
	  ilet = otmp->invlet;
	  if (hallu || !flags.sortpack || otmp->oclass == *invlet) {
	    if (!hallu && flags.sortpack && !classcount) {
	      any.a_void = 0;             /* zero */
#ifdef DUMP_LOG
              if (want_disp)
#endif
