...
struct obj *wep;	/* uwep for attack(), null for kick_monster() */
{
	 ...
	 */
	if ((!canspotmon(mtmp) ||
	     (displaced(mtmp) && m_img_at(u.ux+u.dx, u.uy+u.dy) != mtmp &&
	      !tp_sensemon(mtmp))) &&
		    !glyph_is_warning(glyph_at(u.ux+u.dx,u.uy+u.dy)) &&
		    !glyph_is_invisible(levl[u.ux+u.dx][u.uy+u.dy].glyph) &&
		    !(!Blind && mtmp->mundetected && hides_under(mtmp->data))) {
		if (canspotmon(mtmp))
		    pline("Wait!  That's where %s actually is!",
		          mon_nam(mtmp));
		else
...
/*	Adjust vs. (and possibly modify) monster state.		*/
	...
	if (is_orc(mtmp) && maybe_polyd(is_elf(&youmonst),
			Race_if(PM_ELF)))
	    tmp++;
...
atk_done:
	 ...
	 * evade.
	 */
	if (flags.forcefight && mtmp->mhp > 0 && 
	    (!canspotmon(mtmp) || 
	     (displaced(mtmp) && m_img_at(u.ux+u.dx,u.uy+u.dy) != mtmp &&
	      !tp_sensemon(mtmp))) &&
	    !glyph_is_invisible(levl[u.ux+u.dx][u.uy+u.dy].glyph) &&
...
struct attack *uattk;
{
	...
	if (override_confirmation && wep == uwep) {
	...
	if(!*mhit) {
	    ...
	    /* KMH, conduct */
	    if (wep &&
	        (wep == uwep || (wep == uswapwep && u.twoweap))
...
int thrown;
{
	...
	if(!obj) {	/* attack with bare hands */
	     ...
	     */
	    if (!uarmg) {
		if (uleft && uleft->omaterial == SILVER)
		    barehand_silver_rings++;
		if (uright && uright->omaterial == SILVER)
		    barehand_silver_rings++;
...
rock1:
		...
		if (/* if you strike with a bow... */
	        ...
	        You("jab %s with the %s.", mon_nam(mon),
		                           xname(obj));
	        if (zappable(obj))
		{
		    struct obj *otmp;
		...
		/*scrollhit will handle this if necessary    */
		/*freeinv(obj);*/
		scrollhit(mon, obj, TRUE, TRUE);
		if (mon->mhp <= 0) return FALSE;	/* killed */
		hittxt = TRUE;
		/* in case potion effect causes transformation */
		mdat = mon->data;
...
zombie:
			...
			if (!Slimed) {
			    ...
			    if (!Slimed) {
			    	...
			    	Sprintf(u.uslime_cause, 
			    		"sucking on %s", done_in_name(mdef));
			    }
			    Slimed = 10L;
			}
		    }
		...
		if (!sticks(mdef->data)) {
			...
			if ((is_pool(u.ux,u.uy)) && (!is_swimmer(mdef->data)) &&
			    (!amphibious(mdef->data)) && (!mbreathing(mdef))) {
...
#define oresist_disintegration(obj) \
	    ...
	    case AD_CNCL:
	    	if (!rn2(5))
			(void) cancel_monst(mdef, (struct obj *)0, TRUE,
					    TRUE, FALSE);
		tmp = 0;
	    	break;
	    default:	tmp = 0;
...
{
	register int tmp = /*d((int)mattk->damn, (int)mattk->damd);*/
		d(maybe_polyd(youmonst.data->mlevel, u.ulevel),
		  (int)mattk->damd);
	...
	if(u.uhunger < 1500 && !u.uswallow) {
		...
		switch(mattk->adtyp) {
			...
			if (mdef->mhp > 0) { /* monster lifesaved */
			    ...
			    tmp = 1 + (mons[mons_to_corpse(mdef)].cwt >> 8);
				...
				nomul2(-tmp, "digesting a monster");
				nomovemsg = msgbuf;
			    } else pline("%s", msgbuf);
...
struct monst *mtmp;
{
	...
	if (Blind) {
	    ...
	    /* Wake up the mimic first in case the appearance changes. */
	    wakeup(mtmp, FALSE);
