...
#define MON_IMG_AT(x,y)	(level.monster_images[x][y] != (struct monst *)0 && \
			 !(level.monster_images[x][y])->mburied)
...
#define place_monster(m,x,y)	((m)->mx=(x),(m)->my=(y),\
				 ...
				 displace_mon(m))
#define place_monster_img(m,x,y)	((m)->mix=(x),(m)->miy=(y),\
				 level.monsters[(m)->mix][(m)->miy]=(m))
#endif
#define place_worm_seg(m,x,y)	level.monsters[x][y] = m
#define remove_monster(x,y)	(level.monsters[x][y] = (struct monst *)0, \
                                 level.monster_images[x][y] = (struct monst *)0)
#define remove_monster_img(x,y)	\
    (level.monster_images[x][y] = (struct monst *)0)
#define m_at(x,y)		(MON_AT(x,y) ? level.monsters[x][y] : \
						(struct monst *)0)
#define m_img_at(x,y)		(MON_IMG_AT(x,y) ? level.monster_images[x][y] :\
						(struct monst *)0)
